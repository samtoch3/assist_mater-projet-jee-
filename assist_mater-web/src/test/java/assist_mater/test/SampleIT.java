...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "assist_mater-ear.ear");

        // Current directory is the root of the "assist_mater-web" project. Go up one level
        // and enter the "assist_mater-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../assist_mater-ejb/target/assist_mater-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "assist_mater-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/assist_mater-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "assist_mater-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("assist_mater.test");
